...
def tb_display_generation(writer, step, tag, image):
    ...
    plt.tight_layout()
    plt.imsave(f"./unet_cache/{step}.jpg", image, cmap='gray')
    writer.add_figure(tag, plt.gcf(), global_step=step)


def tb_display_generation_3D(writer, step, tag, image):
    """
    Display generation result in TensorBoard during Diffusion Model training.
    """
    plt.style.use('dark_background')
...
def tb_display_cond_generation(writer, step, tag, starting_image, followup_image, predicted_image):
    ...
    """
    plt.style.use('dark_background')
    _, ax = plt.subplots(ncols=6, nrows=1, figsize=(10, 7))
    for _ax in ax.flatten(): _ax.set_axis_off()

    ax[0].set_title('starting', color='cyan')
    ax[1].set_title('diff', color='cyan')
    ax[2].set_title('follow-up', color='cyan')
    ax[3].set_title('diff', color='cyan')
    ax[4].set_title('prediction', color='cyan')
    ax[5].set_title('diff', color='cyan')
    starting_image = (starting_image - starting_image.min()) / (starting_image.max() - starting_image.min())
    followup_image = (followup_image - followup_image.min()) / (followup_image.max() - followup_image.min())
    predicted_image = (predicted_image - predicted_image.min()) / (predicted_image.max() - predicted_image.min())


    ax[0].imshow(starting_image, cmap='gray')
    diff = followup_image - starting_image
    diff = (diff - diff.min()) / (diff.max() - diff.min())
    ax[1].imshow(diff, cmap="viridis")
    ax[2].imshow(followup_image, cmap='gray')
    diff = followup_image - predicted_image
    diff = (diff - diff.min()) / (diff.max() - diff.min())
    ax[3].imshow(diff, cmap='viridis')
    ax[4].imshow(predicted_image, cmap='gray')
    diff = starting_image - predicted_image
    diff = (diff - diff.min()) / (diff.max() - diff.min())
    ax[5].imshow(diff, cmap='viridis')
    ...
    plt.tight_layout()
    # writer.add_figure(tag, plt.gcf(), global_step=step)
    os.makedirs("./cnet_cache", exist_ok=True)
    plt.savefig(f"./cnet_cache/{step}_{tag}.jpg")

def tb_display_cond_generation_3D(writer, step, tag, starting_image, followup_image, predicted_image):
    """
    Display conditional generation result in TensorBoard during ControlNet training.
    """
    plt.style.use('dark_background')
