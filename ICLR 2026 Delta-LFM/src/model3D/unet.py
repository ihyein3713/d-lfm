...
class DiffusionModelUNet(nn.Module):
    ...
    def __init__(
        ...
        # ---- continuous conditioning MLP (cond_dim -> time_embed_dim), zero-init = no-op at start ----
        _cd = int(cond_dim) if cond_dim else 0
        if _cd > 0:
            _last = nn.Linear(time_embed_dim, time_embed_dim)
            nn.init.zeros_(_last.weight); nn.init.zeros_(_last.bias)
            self.cond_mlp = nn.Sequential(nn.Linear(_cd, time_embed_dim), nn.SiLU(), _last)
        else:
            self.cond_mlp = None

        # ---- SPADE modulation heads (one 1x1x1 conv per level: history maps -> [gamma, beta]), final layer zero-initialized ----
        _sd = int(spade_dim) if spade_dim else 0
        if _sd > 0:
            import torch.nn as _nn
            _chs = [channels[0]] + list(channels)
            _mods = []
            for _c in _chs:
                _l = _nn.Conv3d(32, 2 * _c, 1)   # takes the preceding 32 channels, not the input channel count
                _nn.init.zeros_(_l.weight); _nn.init.zeros_(_l.bias)
                _mods.append(_nn.Sequential(_nn.Conv3d(_sd, 32, 3, padding=1), _nn.SiLU(), _l))
            self.spade_mod = _nn.ModuleList(_mods)
        else:
            self.spade_mod = None
        ...
        y_cond_dim = [10, 64, 128]  
        self.y_embedder = nn.Sequential(
            nn.Linear(
                in_features=y_cond_dim[0],  # e.g., 4 if using latent space from VAE
                out_features=y_cond_dim[0],
            ), nn.SiLU(),

            nn.Linear(
                in_features=y_cond_dim[0],
                out_features=y_cond_dim[1],
            ), nn.SiLU(),

            nn.Linear(
                in_features=y_cond_dim[1],
                out_features=y_cond_dim[2],
            ), nn.SiLU(),
        )
    ...
    def forward(
        ...
        spade_map: torch.Tensor | None = None,
    ) -> torch.Tensor:
        """
        Args:
        ...
        # continuous patient/interval conditioning: MLP -> ADD to time embedding
        # (modulates EVERY resblock; replaces degenerate 1-token cross-attn / discrete class lookup)
        if cond_vec is not None and getattr(self, "cond_mlp", None) is not None:
            emb = emb + self.cond_mlp(cond_vec.to(dtype=emb.dtype))
        ...
        # print("emb = ", emb.shape, class_labels.shape, context.shape)

        # 3. initial convolution
        h = self.conv_in(x)

        # ---- SPADE multi-scale spatial modulation: h <- (1+gamma_l(m_l))*h + beta_l(m_l), where m_l is the history map pooled to that level's resolution ----
        # concat injects only at the first layer and relies on the network to propagate the information downward;
        # SPADE re-injects spatial information at every resolution level. History contributes spatial localization
        # (global pooling in cond removes the benefit). gamma/beta are zero-initialized in the final layer, so the starting point is the identity and warm-starting is safe.
        def _spade(_h, _lvl):
            if spade_map is None or getattr(self, "spade_mod", None) is None: return _h
            if _lvl >= len(self.spade_mod): return _h
            _m = torch.nn.functional.interpolate(spade_map, size=_h.shape[2:], mode="trilinear", align_corners=False)
            _gb = self.spade_mod[_lvl](_m)
            _g, _b = _gb.chunk(2, dim=1)
            return _h * (1.0 + _g) + _b
        ...
        for _li, downsample_block in enumerate(self.down_blocks):
